...
const checkEmailConfirm = function () {
  emailConfirm.value =
    credentialsCheck.email === credentials.email && checkEmail();
};
...
const checkPasswordConfirm = function () {
  passwordConfirm.value =
    credentials.password === credentialsCheck.password && passwordGood.value;
};
...
const checkUsername = function () {
  usernameGood.value =
    ...
    credentials.username.length <= 25 &&
...
const onSubmit = function () {
  if (!emailConfirm.value) {
    ...
    alert(
      "Username must be between 5 and 25 characters, and can't include '@'."
    );
...
<template>
  ...
  <div
    v-if="popup"
    class="fixed top-0 left-0 w-full h-full bg-black bg-opacity-85 z-10"
  ></div>
  <div
    v-if="popup"
    class="fixed top-1/2 transform -translate-y-[50%] left-1/2 -translate-x-[50%] bg-bgpopup w-80 h-[75dvh] md:w-96 z-10 rounded-2xl border border-2 border-brand flex flex-col items-center justify-evenly z-20"
  >
    <div class="px-12 pb-6 md:text-lg text-brand">
      <h1 class="flex justify-center text-xl md:text-2xl font-bold mb-2">
        Note:
      </h1>
      <p>
        - <b><i>Password</i></b> should be between <b>8</b> and
        <b>20</b> characters and contain at least one <b>uppercase</b> letter,
        one <b>lowercase</b> letter, one <b>number</b> and one
        <b>special character</b>
      </p>
      <p>
        - <b><i>Username</i></b> should be between <b>5</b> and
        <b>25</b> characters, and <b>cannot</b> contain <b>@</b>
      </p>
    </div>
  ...
  <!-- signup -->
  <div
    class="h-auto w-auto bg-bgform border-brand border-4 rounded-3xl absolute left-1/2 transform -translate-x-1/2 top-1/2 -translate-y-1/2 flex flex-col mainContainer"
  >
    <h1 class="text-center text-4xl font-bold text-brand mt-6 title">
      Sign Up
    </h1>
    <p class="text-center text-brand mt-6 subtitle">
      Sign up to save your results!
    </p>
    <form
      @submit.prevent="onSubmit"
      class="mt-6 mx-4 md:grid lg:grid-cols-7 gap-x-4 gap-y-6 items-center formContainer"
    >
      ...
      <input
        v-model="credentialsCheck.email"
        @input="checkEmailConfirm"
        name="confirmEmail"
        type="email"
      ...
      <input
        v-model="credentialsCheck.password"
      ...
      <button
        type="submit"
        class="w-36 h-14 border-bg border-2 bg-brand text-bg rounded-full col-span-7 mx-auto text-xl font-medium max-lg:block max-lg:mt-4 lg:hover:bg-bgbtn lg:hover:text-wg lg:hover:border-brand lg:active:scale-98 signupBtn"
      >
        Sign Up!
      </button>
    </form>
    ...
    <h2
      class="mx-auto text-2xl text-brand mt-12 max-lg:block max-lg:mt-8 secondSubtitle"
    >
      Or sign up with:
    </h2>
    ...
    <router-link :to="{ name: 'home' }"
      ><button
        class="w-32 h-12 bg-bgbtn border-2 border-brand mt-10 mx-auto rounded-full text-wg text-xl mb-4 block lg:hover:bg-brand lg:hover:border-bg lg:hover:text-bg ;lg:active:scale-98 quitBtn"
      >
        Quit
      </button></router-link
    >
