...
const chooseAnswer = function(option) {
    usersChoice.value = option;
}
const nextQ = () => {
    if(storeQuiz.questions[curr.value].answer === usersChoice.value) storeQuiz.correctAnswers++;
    if(curr.value < storeQuiz.questions.length-1) curr.value++;
    console.log(storeQuiz.correctAnswers);
}
const prevQ = () => {
...
// TODO: create a new array in storeQuiz to track past choices of the user, so that the user can go back and still see their past answers
</script>
...
<template>
    ...
    <div class="grid grid-cols-1 gap-y-12 mt-16 md:grid-cols-2 md:gap-y-28 md:mt-24 md:px-12 lg:px-20 2xl:px-36 place-items-center">
        ...
        :key="option"
        :class="usersChoice === option ? 'chosenAnswer' : ''"
    ...
    <div class="grid grid-cols-2 place-items-center mt-10 mx-40 md:mx-16 md:mt-16 lg:mx-20 2xl:mx-36">
        ...
        <NavBtn :class="curr === storeQuiz.questions.length-1 ? 'disabled active:scale-[1] disableBtn': ''" @click="nextQ">Next</NavBtn>
    </div>
